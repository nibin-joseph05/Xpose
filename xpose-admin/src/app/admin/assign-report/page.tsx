...
export default function AssignReportPage() {
  ...
  const [newAdminStatus, setNewAdminStatus] = useState<'PENDING' | 'APPROVED' | 'REJECTED' | 'ASSIGNED' | null>(null);
  ...
  const getNearbyOfficers = (report: CrimeReport) => {
    console.log('🔍 getNearbyOfficers called for report:', report.reportId);
    console.log('📋 Report police station:', report.policeStation);
    console.log('🏢 Available stations:', stations);
    console.log('👮 Available officers:', officers);

    if (!report.policeStation) {
      console.log('❌ No police station in report');
      return officers;
    }

    const normalizedReportStation = report.policeStation.toLowerCase().trim();
    console.log('🔍 Normalized station name:', normalizedReportStation);

    const reportStation = stations.find((s) =>
      s.name.toLowerCase().trim() === normalizedReportStation
    );

    if (!reportStation) {
      console.log('❌ No matching station found in stations list');
      console.log('Available station names:', stations.map(s => s.name));
      return officers;
    }

    console.log('✅ Found matching station:', reportStation);

    const stationOfficers = officers.filter((o) => o.stationId === reportStation.id);
    console.log(`👮 Found ${stationOfficers.length} officers for station ${reportStation.name}`);
    console.log('Officers details:', stationOfficers);

    return stationOfficers;
  };

  const handleUpdateStatus = async () => {
    if (!selectedReport || !newAdminStatus) return;

    try {
      console.log('Updating status for report:', selectedReport.reportId, 'to:', newAdminStatus);
      ...
      const response = await fetch(`${API_URL}/api/reports/update-admin-status`, {
        ...
        body: JSON.stringify({
          ...
          reviewedById: 1,
          rejectionReason: newAdminStatus === 'REJECTED' ? rejectionReason : null,
          assignedOfficerId: newAdminStatus === 'ASSIGNED' ? selectedOfficerId : null
        }),
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to update admin status to ${newAdminStatus}: ${errorText}`);
      }

      const result = await response.json();
      console.log('Update successful:', result);

      setSuccessMessage(`Admin status updated successfully to ${newAdminStatus}!`);
      setIsModalOpen(false);
      setSelectedReport(null);
      setSelectedOfficerId(null);
      setNewAdminStatus(null);
      setRejectionReason('');
      await fetchData();
    } catch (err: any) {
  ...
  const handleOpenManageModal = (report: CrimeReport) => {
    ...
    setNewAdminStatus(report.adminStatus as any);
          ...
          {isModalOpen && selectedReport && (
            ...
            >
              ...
              >
                ...
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-400 light:text-gray-600">Current Status: {selectedReport.adminStatus}</label>
                </div>
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-400 light:text-gray-600">New Admin Status</label>
                  <select
                    value={newAdminStatus || ''}
                    onChange={(e) => setNewAdminStatus(e.target.value as any)}
                ...
                {newAdminStatus === 'ASSIGNED' && (
                  <div className="mb-4">
                    <label className="block text-sm font-medium text-gray-400 light:text-gray-600">
                      Select Officer ({officers.length} available)
                    </label>
                    <select
                    ...
                    >
                      <option value="" disabled>
                        {officers.length > 0
                          ? `Select an officer (${officers.length} available)`
                          : 'No officers available'
                        }
                      </option>
                      {officers.map((officer) => (
                        <option key={officer.id} value={officer.id}>
                          {officer.name} - {officer.stationName || 'Unassigned'}
                        </option>
                      ))}
                    </select>

                    {selectedReport.policeStation && (
                      <p className="text-sm text-gray-400 mt-1">
                        Report station: {selectedReport.policeStation}
                      </p>
                    )}
                  </div>
                )}
                ...
                <div className="flex justify-end space-x-2">
                  ...
                  <Button
                    ...
                    disabled={(newAdminStatus === 'ASSIGNED' && !selectedOfficerId) || (newAdminStatus === 'REJECTED' && !rejectionReason)}
